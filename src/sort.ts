...
export const sortProperties = (): void => {
  ...
  let i: number = 0;
  let workspaceIndentation = vscode.workspace
  ...
  while (i < lines.length) {
    ...
    if (
      ...
      properties.sort(
        (a, b) =>
          ORDERED_PROPERTIES.indexOf(a.split(":")[0].trim()) -
          ORDERED_PROPERTIES.indexOf(b.split(":")[0].trim())
      );
      ...
      if (document.languageId === "css") {
        ...
        // scss indentation
        let selectorIndent: string = " ".repeat(
          indentation.length - workspaceIndentation
        );
